...
class Engine(gym.Env, gym.utils.EzPickle):
    ...
    DEFAULT = {
        ...
        'placements_extents': [-2.5, -2.5, 2.5, 2.5],  # Placement limits (min X, min Y, max X, max Y)
    ...
    def __init__(self, config={}, env_params={}):
        ...
        self._wind_gamma = None

        self.parse(self._config)
    ...
    def reset_simulator(self, ):
        ...
        self.modify_physics(self._config, self._env_params)
    ...
    def modify_physics(self, config_sys_, config_):
        
        config_sys = deepcopy(config_sys_)
        config = deepcopy(config_)
        verbose = config.pop("verbose", 0)
        self._render_sys_info = config.pop("render_system_info", True)


        self.render_target_hazard = None

        if 'car' in config_sys['robot_base']:
            '''Temp Disable the car functionality'''
            assert False
            ## car
            # Load the XML file
            self._aug_state = config.pop("aug_state", False)

            # sample env params
            gear_low, gear_high = config.pop("gear_ratio_range", [1.0, 1.0])
            wind_low, wind_high = config.pop("wind_range", [0.0, 0.0])
            wind_change = np.array(config.pop("wind_change", [0,0,0,0,0,0]))
            gear_ratio_1, gear_ratio_2 = np.random.uniform(gear_low, gear_high, size=(2))
            wind_x, wind_y, wind_z, wind_torque_x, wind_torque_y, wind_torque_z = np.random.uniform(wind_low, wind_high, size=(6)) * wind_change
            
            wind_fix = config.pop("wind_fix", None)
            if wind_fix is not None:
                wind_x, wind_y = wind_fix

            wind_list = config.pop("wind_list", None)
            if wind_list is not None:
                '''Sample wind from finite set.'''
                idxWind = np.random.randint(0,len(wind_list))
                wind_x, wind_y = wind_list[idxWind]
            ...
            self._wind_y = wind_y 
            self._wind_z = wind_z 
            self._wind_alpha = wind_torque_x 
            self._wind_beta = wind_torque_y 
            self._wind_gamma = wind_torque_z 

            if verbose:
                print(f"modify_physics | wind: [{self._wind_x, self._wind_y, self._wind_z, self._wind_alpha, self._wind_beta, self._wind_gamma}]")

            # if self._wind_gamma is None:
            #     self._wind_gamma = 0.005        
            # else:
            #     self._wind_gamma *= -1.0

            src_xml_file = os.path.join(BASE_DIR, "xmls/car_bak.xml") 
            dst_xml_file = os.path.join(BASE_DIR, "xmls/car.xml")
            with open(src_xml_file, 'r') as file:
                xml_string = file.read()

            # Convert the XML to a Python dictionary
            xml_dict = xmltodict.parse(xml_string)

            # Modify the gear ratio of the "motor" tags
            for motor in xml_dict['mujoco']['actuator']['motor']:
                # __import__('pdb').set_trace()
                if motor['@name'] == 'left':
                    gear = float(motor['@gear'])
                    motor['@gear'] = str(gear * gear_ratio_1) 

                    if verbose:
                        print(f"modify physics | gear ratio left: {gear_ratio_1}")
                
                if motor['@name'] == 'right':
                    gear = float(motor['@gear'])
                    motor['@gear'] = str(gear * gear_ratio_2) 

                    if verbose:
                        print(f"modify physics | gear ratio right: {gear_ratio_2}")

            # Convert the modified dictionary back to XML
            xml_string_modified = xmltodict.unparse(xml_dict)

            xml_string_modified_pretty = xml.dom.minidom.parseString(xml_string_modified).toprettyxml()


            # Save the modified XML file
            with open(dst_xml_file, 'w') as file:
                file.write(xml_string_modified_pretty)
            
            return

        if 'point' in config_sys['robot_base']:
            ## point 
            # Load the XML file
            self._aug_state = config.pop("aug_state", False)
            ...
            # src_xml_file = os.path.join(BASE_DIR, "xmls/point_bak.xml") 
            # dst_xml_file = os.path.join(BASE_DIR, "xmls/point.xml")
            # with open(src_xml_file, 'r') as file:
            #     xml_string = file.read()
            #
            # # Convert the XML to a Python dictionary
    ...
    def augment_obs(self, obs):
        env_param_obs = np.array([self._gear_ratio_1, self._gear_ratio_2, self._wind_x, self._wind_y, self._wind_z, self._wind_gamma], dtype=np.float32)
        
        # __import__('pdb').set_trace()
        # return np.concatenate((env_param_obs, obs), axis=0)
        return np.concatenate((obs, env_param_obs), axis=0)

    @property
    def wind_xy(self):
        '''Helper to get wind (x, y)'''
        return np.array([self._wind_x, self._wind_y]).copy()
    ...
    @property
    def robot_loc(self):
        return np.concatenate((self.robot_pos[:2], self.robot_vel[:2]),axis=0).copy()

    @property
    def robot_state_pred(self):
    ...
    def build_observation_space(self):
        ...
        # assert self.observation_flatten
        
        if self.observation_flatten:
            self.obs_flat_size = sum([np.prod(i.shape) for i in self.obs_space_dict.values()])
            self.observation_space = gym.spaces.Box(-np.inf, np.inf, (self.obs_flat_size,), dtype=np.float32)
            if self._aug_state:
                self.observation_space = gym.spaces.Box(-np.inf, np.inf, (6 + self.obs_flat_size,), dtype=np.float32)
    ...
    def reset(self):
        ...
        # reset render actions
        self.update_for_render()
    ...
    def obs(self):
        ...
        if self.observe_sensors:
            ...
            for sensor in self.sensors_obs:  # Explicitly listed sensors
                ...
                # print(f"sensor:{sensor} -> {obs[sensor]}")
            for sensor in self.robot.hinge_vel_names:
                obs[sensor] = self.world.get_sensor(sensor)
            for sensor in self.robot.ballangvel_names:
                obs[sensor] = self.world.get_sensor(sensor)
        ...
        if self.buttons_num and self.observe_buttons:
            ...
            if self.buttons_timer == 0:
                ...
                obs['buttons_lidar'] = np.zeros(self.lidar_num_bins, dtype=np.float32)
        if self.observe_qpos:
            obs['qpos'] = self.data.qpos.copy()
            # robo_pos = self.robot_pos
            # obs['_robot_pos'] = obs['qpos'][:len(robo_pos)]
            # obs['qpos'] = obs['qpos'][len(robo_pos):]
            # assert np.all(robo_pos == obs['_robot_pos'])
            # print("pos", self.robot_pos) 
            
        if self.observe_qvel:
            obs['qvel'] = self.data.qvel.copy()
            # robo_vel = self.robot_vel
            # obs['_robot_vel'] = obs['qvel'][:len(robo_vel)]
            # obs['qvel'] = obs['qvel'][len(robo_vel):]
            # assert np.all(obs['_robot_vel'] == robo_vel)
            # print("vel", self.robot_vel)
        if self.observe_ctrl:
    ...
    def step(self, action):
        ...
        # Add external wind to the robot object 
        wind_force = np.array([self._wind_x, self._wind_y, self._wind_z, self._wind_alpha, self._wind_beta, self._wind_gamma])  # A constant wind force in the x-direction
        for i in [self.model.body_name2id("robot")]:
            self.sim.data.xfrc_applied[i, :] = wind_force
    ...
    def update_for_render(self, action = np.array([0.0,0.0]), 
                                action_hat= np.array([0.0, 0.0]), conservative=None):
        self.act = action
        self.act_inc = action_hat
        self.act_conservative = conservative

    def mark_target_hazard(self, loc):
        self.render_target_hazard = loc
    ...
    def render(self,
        ...
        if self.render_target_hazard is not None:
            ## Render target hazard
            self.render_sphere((self.render_target_hazard[0],
                                self.render_target_hazard[1],
                                0.5), 0.5, COLOR_GREMLIN, alpha=.5)
        ...
        if mode=='human':
            ...
            if self._render_sys_info:
                ...
                img = cv2.putText(img, "ACTION [{:.2f}, {:.2f}], GEAR [{:.1f}, {:.1f}]".format(self.act[0], self.act[1], self._gear_ratio_1, self._gear_ratio_2), (10,10), font, fontScale, color, thickness, cv2.LINE_AA)
                img = cv2.putText(img, "CBF [{:.4f}, {:.4f}] ".format(self.act_inc[0], self.act_inc[1]), (10,25), font, fontScale, color, thickness, cv2.LINE_AA)

                # img = cv2.putText(img, "action", org, font, fontScale, color, thickness, cv2.LINE_AA)
                # img = cv2.putText(img, "angle {:.2f}, v {:.2f}".format(self.state[0],self.state[1]), 
                #             (50,80), font, fontScale, color, thickness, cv2.LINE_AA)
                img = cv2.putText(img, "WINDx: [{:.4f}, {:.4f}]".format(self._wind_x,self._wind_y ), (10,40), font, fontScale, color, thickness, cv2.LINE_AA)
                # img = cv2.putText(img, "WINDa: [{:.3f}, {:.3f}, {:.3f}]".format(self._wind_alpha, self._wind_beta, self._wind_gamma), (10,55), font, fontScale, color, thickness, cv2.LINE_AA)
                # if self.act_conservative:
                #     img = cv2.putText(img, "conservative {:.3f}".format(self.act_conservative), 
                #                 (50,140), font, fontScale, color, thickness, cv2.LINE_AA)

                # if self.render_target_hazard is not None:
                #     # __import__('pdb').set_trace()
                #     img = cv2.putText(img, "Target Hazard", 
                #                   self.render_target_hazard, font, fontScale, color, thickness, cv2.LINE_AA)

            self.act_inc = np.zeros(2) 
            self.act_conservative = None
            # return data[::-1, :, :]
            return img
